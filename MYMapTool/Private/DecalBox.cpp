...
CDecalBox::CDecalBox(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CToolObj(pDevice, pContext)
{
}

CDecalBox::CDecalBox(const CDecalBox& rhs)
	: CToolObj(rhs)
{
}

HRESULT CDecalBox::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CDecalBox::Initialize(void* pArg)
{
	...
	if (nullptr != pArg)
	{
		...
		strcpy_s(m_szName, "Prototype_GameObject_DecalBox");
...
void CDecalBox::Priority_Tick(_float fTimeDelta)
{

}

void CDecalBox::Tick(_float fTimeDelta)
{
...
void CDecalBox::Late_Tick(_float fTimeDelta)
{
#ifdef _DEBUG
...
HRESULT CDecalBox::Add_Components(void* pArg)
{
...
HRESULT CDecalBox::Bind_ShaderResources()
{
...
CDecalBox* CDecalBox::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CDecalBox* pInstance = new CDecalBox(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CDecalBox");
...
CGameObject* CDecalBox::Clone(void* pArg)
{
	CDecalBox* pInstance = new CDecalBox(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CDecalBox");
		Safe_Release(pInstance);
	}
