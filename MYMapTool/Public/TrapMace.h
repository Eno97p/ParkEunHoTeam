...
class CTrapMace final : public CToolObj
{
public:
	enum TRIGGER_TYPE { TRIG_TUTORIAL_BOSSENCOUNTER, TRIG_END };
private:
	CTrapMace(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CTrapMace(const CTrapMace& rhs);
	virtual ~CTrapMace() = default;
...
public:
	static CTrapMace* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
