...
HRESULT CWeapon_Arrow_LGGun::Initialize(void* pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_LOOK, XMVectorSet(
		XMVectorGetX(m_pTransformCom->Get_State(CTransform::STATE_LOOK)), 0.f, 
		XMVectorGetZ(m_pTransformCom->Get_State(CTransform::STATE_LOOK)), 0.f));
