...
private:
	...
	_bool	m_isCollderAble = { false }; // Collider È°¼ºÈ­ ¿©ºÎ

	// Imguizmo
	_bool		useWindow = { false };
	_int		gizmoCount = 1;

	_int		m_iFileIdx;					// ÆÄÀÏ Index
	_int		m_iAnimIdx;					// Anim Index
	_int		m_iChannelIdx;				// Channel Index
	_int		m_iNumKeyFrame;				// KeyFrame °³¼ö
	_int		m_iCurrentKeyframe = { 0 }; // ÇöÀç KeyFrame
	...
	_double		m_dMaxDuration;		// ÃÑ Duration
	_double		m_dCurrentPos;		// ÇöÀç Anim À§Ä¡
	_float		m_fStartKeyframe = { 0.0 };
	_float		m_fEndKeyframe = { 0.0 };

	vector<_char*>					m_vecAnim;			// Anim º¤ÅÍ
	vector<_char*>					m_vecChannel;		// Channel º¤ÅÍ
	vector<_char*>					m_vecBone;			// Bone º¤ÅÍ
	vector<const _char*>			m_vecSound;			// Sound º¤ÅÍ
	map<_int, vector<string>>		m_mapApplySound; // const _char*
	vector<const _char*>			m_vecEffect;		// Effect º¤ÅÍ
