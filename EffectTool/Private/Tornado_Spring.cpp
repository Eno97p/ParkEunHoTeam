...
CTornado_Spring::CTornado_Spring(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CBlendObject(pDevice, pContext)
{
}

CTornado_Spring::CTornado_Spring(const CTornado_Spring& rhs)
	:CBlendObject(rhs)
{
}

HRESULT CTornado_Spring::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTornado_Spring::Initialize(void* pArg)
{
	if (pArg == nullptr)
		return E_FAIL;
	m_OwnDesc = make_shared<TORNADO_SPRING>(*((TORNADO_SPRING*)pArg));
...
void CTornado_Spring::Priority_Tick(_float fTimeDelta)
{
	if (EffectDead)
		return;
}

void CTornado_Spring::Tick(_float fTimeDelta)
{
	if (EffectDead)
		return;
...
void CTornado_Spring::Late_Tick(_float fTimeDelta)
{
	if (EffectDead)
		return;
...
HRESULT CTornado_Spring::Render()
{
...
HRESULT CTornado_Spring::Render_Bloom()
{
	if (FAILED(Bind_ShaderResources()))
		return E_FAIL;
	_uint	iNumMeshes = m_pModelCom->Get_NumMeshes();
...
HRESULT CTornado_Spring::Add_Components()
{
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_TornadoSpring"),
...
HRESULT CTornado_Spring::Bind_ShaderResources()
{
...
CTornado_Spring* CTornado_Spring::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTornado_Spring* pInstance = new CTornado_Spring(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CTornado_Spring");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CTornado_Spring::Clone(void* pArg)
{
	CTornado_Spring* pInstance = new CTornado_Spring(*this);
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CTornado_Spring");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CTornado_Spring::Free()
{
	__super::Free();
